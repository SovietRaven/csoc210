...
public class GenericTree<T> implements Tree<T> {
    ...
    protected TreeNode<T> root; // this tree's root
    protected int size; // how many nodes are in the tree
   protected int height; // keep track of the greatest deapth leaf so we dont have to calculate it later!
    public GenericTree() {
        this.root = null;
        this.size = 0;
        this.height = 0;
    ...
    // Will not work if tree has a lot of nodes
    @Override
    ...
    public TreeNode<T> search(T searchdata) {
        ...
        //if we make it here the tree is not empty
    ...
    protected TreeNode<T> searchHelper(T searchdata, TreeNode<T> node) {
        if (node.element().equals(searchdata))
            return node; // we found it!!!! return up the recursive call hiearchy
    ...
    protected TreeNode<T> searchHelperBFS(T searchdata, TreeNode<T> root) {
         if (root.element().equals(searchdata))
            return root; // we found it!!!! return up the recursive call hiearchy
    ...
    protected TreeNode<T> searchLevel(T searchdata, TreeNode<T> children[]) {
        ...
        if (children == null) {
            return null;// we made it to the end and never found it
        }
        ...
        for (int i = 0; i < children.length; i++) {
            ...
            if (child.children()!= null) {
                ...
                for (int j=0; j<grandchildren.length; j++) {
                    cousins.add(grandchildren[j]);//ssssssssssssssssssssss
                }
            }
        }
       // step 2 - recursively search the grandchildren (cousins)
